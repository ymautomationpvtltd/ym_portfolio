...
export default function VentureShowcase({ theme }) {
  ...
  const venturePillars = [
    {
      ...
      icon: Wifi,
      color: isDark ? "from-cyan-500 to-blue-500" : "from-[#015CCF] to-blue-500"
    },
    {
      id: "mechatronics",
    ...
    },
    {
      ...
      icon: Activity,
      color: isDark ? "from-indigo-500 to-purple-500" : "from-indigo-600 to-blue-700"
    },
    {
      id: "itot",
      ...
      icon: Server,
      color: isDark ? "from-emerald-500 to-cyan-500" : "from-emerald-600 to-[#015CCF]"
    }
  ];

  const architectureSteps = [
    { name: "1. Sensor Layer", desc: "Vibration, Thermal & Current Sensors", status: "Active Real-Time", icon: Radio, color: isDark ? "border-cyan-500 text-cyan-400" : "border-[#015CCF] text-[#015CCF]" },
    { name: "2. Edge PLC / Micro", desc: "Siemens/Rockwell Logic Controllers", status: "Sub-millisecond", icon: Cpu, color: isDark ? "border-blue-500 text-blue-400" : "border-blue-600 text-blue-600" },
    { name: "3. IIoT Gateway", desc: "OPC-UA & MQTT Security Bus", status: "Encrypted Stream", icon: Wifi, color: isDark ? "border-indigo-500 text-indigo-400" : "border-indigo-600 text-indigo-600" },
    { name: "4. Cloud Analytics", desc: "OEE, Predictive Insights & Alerts", status: "Live Dashboard", icon: MonitorCheck, color: isDark ? "border-emerald-500 text-emerald-400" : "border-emerald-600 text-emerald-600" },
  ];
  ...
  return (
    <section id="venture" className={`py-24 relative overflow-hidden transition-colors duration-300 ${
      ...
      {/* Background Mesh */}
      <div className={`absolute inset-0 bg-grid-mesh pointer-events-none ${isDark ? 'opacity-30' : 'opacity-50'}`} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-16">
          <div className={`p-5 rounded-2xl border text-center space-y-1 ${
            isDark ? 'glass-card border-slate-800' : 'bg-white border-blue-200 shadow-sm'
          }`}>
            ...
            <p className={`text-[10px] font-mono font-bold ${isDark ? 'text-cyan-400' : 'text-[#015CCF]'}`}>Inc. Aug 31, 2020</p>
          </div>
          ...
          <div className={`p-5 rounded-2xl border text-center space-y-1 ${
            ...
            <p className={`text-[10px] font-mono font-bold ${isDark ? 'text-cyan-400' : 'text-[#015CCF]'}`}>Tamil Nadu, India</p>
          </div>
          ...
          <div className={`p-5 rounded-2xl border text-center space-y-1 ${
            ...
            <p className={`text-[10px] font-mono font-bold ${isDark ? 'text-cyan-400' : 'text-[#015CCF]'}`}>Managing Director</p>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`p-2.5 rounded-xl text-white bg-gradient-to-br ${pillar.color} shadow-md`}>
                      <IconComp className="w-5 h-5" />
                    </div>
                    <div>
                      <h4 className={`text-sm font-bold transition-colors ${
                        isSelected 
                          ? isDark ? 'text-cyan-300' : 'text-[#015CCF]'
                          : isDark ? 'text-white group-hover:text-cyan-300' : 'text-slate-900 group-hover:text-[#015CCF]'
                      }`}>
                        {pillar.title}
                      </h4>
                  ...
                  <ArrowRight className={`w-4 h-4 transition-transform ${
                    isSelected 
                      ? isDark ? 'text-cyan-400 translate-x-1' : 'text-[#015CCF] translate-x-1' 
              ...
              <div className={`rounded-3xl p-8 border space-y-6 relative overflow-hidden ${
                ...
                <div className={`flex items-center justify-between border-b pb-4 ${
                  ...
                  <div className={`p-3 rounded-2xl border ${
                    isDark ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20' : 'bg-[#015CCF]/10 text-[#015CCF] border-[#015CCF]/20'
                  }`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {venturePillars[activeTab].features.map((feat, i) => (
                      <div key={i} className={`flex items-center gap-2 p-3 rounded-xl border text-xs font-medium ${
                        ...
                        <CheckCircle2 className={`w-4 h-4 shrink-0 ${isDark ? 'text-cyan-400' : 'text-[#015CCF]'}`} />
                        <span>{feat}</span>
                      </div>
                    ))}
                  </div>
            ...
            {architectureSteps.map((step, idx) => {
              ...
              return (
                <div key={idx} className={`relative p-5 rounded-2xl border space-y-3 ${
                  isDark ? 'bg-slate-900/80 border-slate-800' : 'bg-blue-50/40 border-blue-200'
                }`}>
                  <div className="flex items-center justify-between">
                    <div className={`p-2 rounded-xl border ${step.color} ${isDark ? 'bg-slate-800' : 'bg-white'}`}>
                  ...
                  <div className={`pt-2 border-t flex items-center justify-between text-[10px] font-mono ${
                    ...
                    <span className={`font-bold ${isDark ? 'text-cyan-400' : 'text-[#015CCF]'}`}>{step.status}</span>
                  </div>
                </div>
              );
            })}
