  ...
  return (
    <section id="investor" className={`py-24 relative overflow-hidden border-t transition-colors duration-300 ${
      ...
      <div className={`absolute top-1/3 right-0 w-[500px] h-[500px] blur-[140px] rounded-full pointer-events-none ${
        isDark ? 'bg-cyan-500/10' : 'bg-[#015CCF]/10'
      }`} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          ...
          <h2 className={`text-3xl sm:text-4xl font-extrabold tracking-tight font-sans ${isDark ? 'text-white' : 'text-slate-900'}`}>
            Why Invest in <span className={isDark ? 'text-gradient-cyan' : 'text-gradient-primary'}>YM Automation</span>?
          </h2>
            ...
            return (
              ...
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className={`p-3 rounded-2xl ${
                      isDark ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 'bg-[#015CCF]/10 text-[#015CCF]'
                    }`}>
                  ...
                  <div>
                    ...
                    <div className={`text-xs font-mono font-bold ${isDark ? 'text-cyan-400' : 'text-[#015CCF]'}`}>{item.sub}</div>
                  </div>
                ...
                <div className={`pt-3 border-t text-[11px] font-mono flex items-center justify-between ${
                  isDark ? 'border-slate-800 text-slate-400' : 'border-slate-100 text-slate-500 font-medium'
                }`}>
                  <span>Investment Grade</span>
                  <span className={`font-bold ${isDark ? 'text-cyan-400' : 'text-[#015CCF]'}`}>✓ Verified</span>
                </div>
              </div>
            );
          })}
        ...
        <div className={`rounded-3xl p-8 border space-y-8 mb-12 ${
          ...
          <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-6 ${
            ...
            <button
              ...
              className={`px-5 py-3 rounded-xl text-white font-bold text-xs shadow-lg transition-all flex items-center gap-2 ${
                isDark 
                  ? 'bg-gradient-to-r from-cyan-500 to-blue-600 shadow-cyan-500/25 hover:shadow-cyan-500/40' 
                  : 'bg-[#015CCF] hover:bg-[#014ea4] shadow-[#015CCF]/20'
              }`}
            >
        ...
        <div className={`relative rounded-3xl p-8 sm:p-10 border text-white shadow-2xl overflow-hidden ${
          isDark 
            ? 'bg-gradient-to-r from-slate-900 via-slate-900 to-cyan-950/60 border-cyan-500/30' 
            : 'bg-gradient-to-r from-[#015CCF] via-blue-700 to-blue-800 border-blue-400'
        }`}>
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-8 space-y-3 text-left">
              <span className={`px-3 py-1 rounded-full text-xs font-mono font-bold border ${
                isDark ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/30' : 'bg-white/20 text-white border-white/30'
              }`}>
            ...
            <div className="lg:col-span-4 flex flex-col sm:flex-row lg:flex-col gap-3">
              <button
                onClick={onOpenInvestorModal}
                className={`w-full py-3.5 px-6 rounded-xl font-bold text-xs text-center flex items-center justify-center gap-2 shadow-lg transition-all ${
                  isDark 
                    ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-cyan-500/20 hover:scale-[1.02]' 
              ...
              <a
                ...
                className={`w-full py-3.5 px-6 rounded-xl font-semibold text-xs text-center flex items-center justify-center gap-2 transition-all ${
                  isDark 
                    ? 'bg-slate-950 border border-slate-700 hover:border-cyan-500 text-slate-200' 
                    : 'bg-[#013e84] border border-blue-400 text-white hover:bg-[#04356c]'
                }`}
              >
                <Zap className={`w-4 h-4 ${isDark ? 'text-cyan-400' : 'text-cyan-300'}`} />
